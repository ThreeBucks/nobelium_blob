...
export async function getAllPosts ({ includePages = false }) {
  ...
  const databaseCollectionId = rawMetadata?.collection_id
  const collection = databaseCollectionId
    ? response.collection?.[databaseCollectionId]?.value
    : Object.values(response.collection || {})[0]?.value
  const collectionQuery = databaseCollectionId && response.collection_query?.[databaseCollectionId]
    ? { [databaseCollectionId]: response.collection_query[databaseCollectionId] }
